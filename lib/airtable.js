...
// Read env at call time, not module load, so a function instance that was
// warm before env vars were added still picks them up after redeploy.
function cfg() {
  return {
...
function assertConfig() {
  const { BASE_ID, TABLE_ID, API_KEY } = cfg();
  const missing = [
  ...
  if (missing.length) {
    throw new Error(`Airtable env not configured: missing ${missing.join(', ')}`);
  }
}
